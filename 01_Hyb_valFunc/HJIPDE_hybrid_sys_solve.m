...
for i = istart:length(tau)
    ...
    %%-------Updated April 2024 by Javier Borquez (javierbo@usc.edu)

    %split value function vector on individual modes
    len_cont_state = length(y)/schemeData.nq;
    y_qi=reshape(y,len_cont_state,schemeData.nq);
    
    %temporarily save grid, l(x), and data array with cont+discrete states
    old_g = g;
    old_data0=data0;
    old_y =y;
    %array used to project only over the discrete modes
    keep_dim = ones(1,ndims(data0));
    keep_dim(end) = 0; 
    
    for qi = 1:schemeData.nq        
        
        %select grid for cont dyn and corresponding l(x)
        [g, data0] = proj(old_g, old_data0, ~keep_dim, qi);
        schemeData.grid = g;   

        %select the appropiate continuous model for each qi 
        schemeData.dynSys = schemeData.system_list{1, qi};        
        y = y_qi(:,qi);
        tNow = tau(i-1);
        ...
        while tNow < tau(i) - small
            ...
            %% Here's where we do the min/max for BRTS or nothing for BRSs.              
            %   compMethod
            ...
            elseif strcmp(compMethod, 'minVWithV0')%Min with data0
                y = min(y,data0(:));
            elseif strcmp(compMethod, 'minDog1D')
                y = min(y,data0(:));
            elseif strcmp(compMethod, 'minAirColl3D')
                y = min(y,data0(:));
            elseif strcmp(compMethod, 'minJump4D')
                y = min(y,data0(:));              
            ...
            elseif strcmp(compMethod, 'minVWithL') ...
                ...
                error('compMethod not found in integration loop')
            end
    ...
    %%back to full size grid and data array on exiting integration loop
    g= old_g;
    schemeData.grid = g;
    data0 = old_data0;
    y=old_y;
    y= reshape(y_qi,length(y),1);

    %%apply jump conditions and reset maps
    if strcmp(compMethod, 'minDog1D')
        H = prod(data0size)/data0size(end); %len of value-fcn array for qi
        %forced switch
        y= y(schemeData.reset_map_q13);
        %control switch
        mask1=[ones(H,1);NaN(2*H,1)];
        mask2=circshift(mask1,H);
        %mask3=circshift(mask2,H);
        y1to2 = y(schemeData.reset_map_q12).*mask1;
        y2to1 = y(schemeData.reset_map_q21).*mask2;
        %y3to2 = y(schemeData.reset_map_q32).*mask3;
        y = min([y,y1to2,y2to1],[],2);
        %Min with data0
        y = min(y,data0(:));
    elseif strcmp(compMethod, 'minAirColl3D')
        H = prod(data0size)/data0size(end); %len of value-fcn array for qi
        %control switch
        mask1to2=[ones(H,1);NaN(2*H,1)];
        y1to2 = y(schemeData.reset_map_q12).*mask1to2;
        y = max([y,y1to2],[],2);
        %forced switch
        y= y(schemeData.reset_map_q23);
        %Min with data0
        y = min(y,data0(:));
     elseif strcmp(compMethod, 'minJump4D')
        H = prod(data0size)/data0size(end); %len of value-fcn array for qi
        ...
        %forced switch
        y= y(schemeData.reset_map_q12);
        y= y(schemeData.reset_map_q21);
        y= y(schemeData.reset_map_q13);
        y= y(schemeData.reset_map_q23);
        %Min with data0
        y = min(y,data0(:));
     else
        error('compMethod not found in reset maps')
     end

    % Reshape value function
    data_i = reshape(y, g.shape);

    if strcmp(compMethod, 'min_hopper')
        extraOuts.qi = [extraOuts.qi I]; 
    end
