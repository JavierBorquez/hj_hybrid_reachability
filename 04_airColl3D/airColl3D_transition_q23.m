...
function [reset_map, params] = roundabout_transition_q23(grid, params)
...
params.state_fcn_arr_q23 = state_fcn_arr;
...
function x3_post = x_f3(x,grid)
  [~,zpi_idx]=min(abs(grid.vs{3, 1}-pi));
  x3_post = grid.vs{3,1}(zpi_idx + 1);
end

% state jump rule for 4th dim, q config
function x4_post = x_f4(x,grid)
  x4_post = 3; %land in q3
end
...
function is_reset = resetmap_trigger_event(grid, i_t)
  ...
  if (x_ref(4)==2 && x_ref(3)>pi) %if in q2 and z>pi
    is_reset = 1;
  end
