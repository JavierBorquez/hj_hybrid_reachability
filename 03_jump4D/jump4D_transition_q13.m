...
function [reset_map, sys] = dog_jump_onlyReach_transition_q13(grid, sys)
...
sys.params.state_fcn_arr_q13 = state_fcn_arr;%%<----------------dont forget to rename
...
function is_reset = resetmap_trigger_event(grid, i_t, sys)
  ...
  in_obstacle = (x_ref(1)>3 && x_ref(1)<4 && x_ref(2)<1);
  % check based on current state
  if (x_ref(5)==1 && (x_ref(2)<0 || x_ref(2)>1.5*sys.l0 || in_obstacle))  % took out the ground (x_ref(2)<0 || )
    is_reset = 1;
  end
